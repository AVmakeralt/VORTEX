...
int main(void) {
    ...
    /* Print the instruction stream with pos, focusing on CMP and JCC */
    fprintf(stderr, "=== Instructions around second If ===\n");
    for (uint32_t b = 0; b < stream->block_count; b++) {
        for (uint32_t i = 0; i < stream->blocks[b].inst_count; i++) {
            ...
            if (inst->opcode == 18 /*CMP*/ || inst->opcode == 38 /*JCC*/ ||
                (inst->opcode == 20 /*MOV*/ && (inst->flags & 0x1) && inst->imm == 0x7FF8000000000000ULL)) {
                const char *opname = inst->opcode == 20 ? "MOV" : inst->opcode == 18 ? "CMP" : "JCC";
                fprintf(stderr, "  B%u [%u] pos=%u %s k0=%u op0=%u k1=%u op1=%u\n",
    ...
    if (ra) {
        /* Print intervals sorted by start */
        fprintf(stderr, "\n=== All intervals sorted by start ===\n");
        for (uint32_t v = 0; v < ra->interval_count; v++) {
            vtx_live_interval_t *iv = &ra->intervals[v];
            fprintf(stderr, "  vreg %u: start=%u end=%u phys=%u\n",
                   iv->vreg, iv->start, iv->end, iv->phys_reg);
        }
    }
