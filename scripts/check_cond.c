...
int main(void) {
    ...
    // Save pre-opt If conds
    for (uint32_t i = 0; i < graph.node_table.count; i++) {
        vtx_node_t *n = &graph.node_table.nodes[i];
        if (n->dead) continue;
        if (n->opcode == VTX_OP_If) printf("PRE-OPT N%u: If cond=%u\n", i, n->cond);
    }
    /* Print If nodes with cond values */
    uint32_t saved_count = graph.node_table.count; for (uint32_t i = 0; i < graph.node_table.count; i++) {
        vtx_node_t *n = &graph.node_table.nodes[i];
        if (n->dead) continue;
        if (n->opcode == VTX_OP_If || n->opcode == VTX_OP_Proj) {
            printf("  N%u: %s cond=%u -> [", i, vtx_node_opcode_name(n->opcode), n->cond);
            for (uint32_t j = 0; j < n->input_count; j++)
                printf("N%u%s", n->inputs[j], j+1<n->input_count?",":"");
            printf("]\n");
        }
    }
    /* Print Region nodes with their inputs */
    uint32_t saved_count = graph.node_table.count; for (uint32_t i = 0; i < graph.node_table.count; i++) {
        vtx_node_t *n = &graph.node_table.nodes[i];
        if (n->dead) continue;
        if (n->opcode == VTX_OP_Region) {
            printf("  N%u: Region -> [", i);
            for (uint32_t j = 0; j < n->input_count; j++)
                printf("N%u%s", n->inputs[j], j+1<n->input_count?",":"");
            printf("]\n");
