/* Test: does T2 correctly handle if_true? */
#include <stdio.h>
#include <stdint.h>
#include <string.h>
...
int main(void) {
    /* Test 1: if_true — if arg is truthy, return 1, else return 0 */
    const char *if_true_prog =
        ".method if_true_test (I)I\n"
        ".arg_count 1\n"
        "load_local 0\n"
        "if_true truthy\n"
        "load_const_int 0\n"
    ...
    printf("=== if_true test ===\n");
    vtx_value_t r1 = run_jit(if_true_prog, vtx_make_smi(5));
    printf("if_true(5) = 0x%016llX (expected SMI(1)=0x%016llX)\n",
           (unsigned long long)r1, (unsigned long long)vtx_make_smi(1));
    vtx_value_t r2 = run_jit(if_true_prog, vtx_make_smi(0));
    printf("if_true(0) = 0x%016llX (expected SMI(0)=0x%016llX)\n",
           (unsigned long long)r2, (unsigned long long)vtx_make_smi(0));

    /* Test 2: if_false — if arg is falsy, return 1, else return 0 */
    const char *if_false_prog =
        ".method if_false_test (I)I\n"
        ".arg_count 1\n"
        "load_local 0\n"
        "if_false falsy\n"
        "load_const_int 0\n"
    ...
    printf("\n=== if_false test ===\n");
    vtx_value_t r3 = run_jit(if_false_prog, vtx_make_smi(5));
    printf("if_false(5) = 0x%016llX (expected SMI(0)=0x%016llX)\n",
           (unsigned long long)r3, (unsigned long long)vtx_make_smi(0));
    vtx_value_t r4 = run_jit(if_false_prog, vtx_make_smi(0));
    printf("if_false(0) = 0x%016llX (expected SMI(1)=0x%016llX)\n",
           (unsigned long long)r4, (unsigned long long)vtx_make_smi(1));
