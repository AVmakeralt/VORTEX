...
- Sea of Nodes IR with around 60 opcodes and 14 optimization passes
- Partial Escape Analysis with cross-object scalar replacement
- Representation inference using explicit `UnboxInt`/`BoxInt` nodes for SMI tag elision
...
- Concurrent compilation using a pthread threadpool, orchestrator, and safepoints
- Hand-written x86-64 emitter
...
- Heap-allocated artifacts, allowing AOT artifacts to survive arena scope and enabling background compilation
- Aggressive optimization: `inline_size_limit=8192` (vs. 4096 JIT), GVN 5 iterations (vs. 3), SCCP 10 iterations (vs. 5), speculative guards, loop specialization, SIMD vectorization
- Bailout stubs, pre-generated code that reconstructs interpreter state on guard failure
- Guard failure and retracing, failed guards feed into the trace re-tracing system to generate a new specialized variant
- Code installation through `vtx_install_method`
...
// Worker thread compiles and installs automatically
// Guard failures feed into the retrace system
```
...
- Per-method failure tracking, using a hash table to record guard failures per method
- Threshold-based re-tracing, 10+ failures trigger a re-trace
- Profile feedback, guard failures update branch probabilities so the next trace selects a different hot path
- Cooldown, 10 orchestrator checks between re-traces prevent storms
- Maximum of 5 re-traces per method before giving up
...
Adding a new architecture only requires implementing a `TargetDescription`, not rewriting the register allocation algorithm.
...
This codebase was originally written by a human developer and later modified by an AI assistant (GLM/Z.ai) for bug fixes, performance improvements, and feature additions. Modified files carry an **"AI-MODIFIED CODE"** banner. See the commit history for detailed change descriptions.
